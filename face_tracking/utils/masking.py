...
class Masking(nn.Module):
    def __init__(self, flame_assets = None):

        self.flame_assets = flame_assets

        dir = f'{self.flame_assets}'
        super(Masking, self).__init__()
        flame_masks_path = f'{dir}/FLAME2020/FLAME_masks/FLAME_masks.pkl'
    ...
    def get_masked_faces(self):
        if self.masked_faces is None:
            ...
            self.masked_faces = torch.from_numpy(np.array(m.faces)).to(get_device()).long()[None]
    ...
    def get_masked_mesh(self, vertices, triangle_mask):
        ...
        faces = self.faces.cpu().numpy()
        masked_vertices = torch.empty(0, 0, 3).to(get_device())
        masked_faces = torch.empty(0, 0, 3).to(get_device())
        for i in range(B):
            m = Trimesh(vertices=vertices[i].detach().cpu().numpy(), faces=faces, process=False)
            m.update_faces(triangle_mask)
            m.process()
            f = torch.from_numpy(np.array(m.faces)).to(get_device())[None]
            v = torch.from_numpy(np.array(m.vertices)).to(get_device())[None].float()
            if masked_vertices.shape[1] != v.shape[1]:
                masked_vertices = torch.empty(0, v.shape[1], 3).to(get_device())
            if masked_faces.shape[1] != f.shape[1]:
                masked_faces = torch.empty(0, f.shape[1], 3).to(get_device())
